...
def train(args,device_id):
    ...
    padding_idx = args.tokenizer.pad_token_id
    symbols = {'BOS': args.tokenizer.vocab['<s>'], 'EOS': args.tokenizer.vocab['</s>'], 'PAD': args.tokenizer.vocab['<pad>'],\
    'UNK': args.tokenizer.vocab['<unk>']}
...
def test(args, pt, step):
    ...
    # vocab = spm
    tokenizer = AutoTokenizer.from_pretrained('/data/run01/scv6131/wpc/Prompt_Contrastive_BCELoss/longformer-base-4096')
    padding_idx = args.tokenizer.pad_token_id
    symbols = {'BOS': args.tokenizer.vocab['<s>'], 'EOS': args.tokenizer.vocab['</s>'], 'PAD': args.tokenizer.vocab['<pad>'],\
    'UNK': args.tokenizer.vocab['<unk>']}
    vocab_size = len(args.tokenizer)
    ...
    model.eval()
    predictor = build_predictor(args, tokenizer, symbols, model, device, args.bce, logger=logger)
...
if __name__ == '__main__':
    ...
    train_text_file = os.path.join(args.data_dir, "train_gptctr2.json")
    
    val_text_file = os.path.join(args.data_dir, "train_gptctr2.json")
    
    test_text_file = os.path.join(args.data_dir, "test_gptctr2.json")
    '''
    ...
    device = "cpu" if args.visible_gpus == '-1' else "cuda"
    args.device =  device
    args.tokenizer = AutoTokenizer.from_pretrained('/data/run01/scv6131/wpc/Prompt_Contrastive_BCELoss/longformer-base-4096')
    ...
    if (args.mode == 'train'):
        ...
        valid_dataset = ExampleSet(val_text_file, args.tokenizer, args.sent_max_len, args.doc_max_timesteps, args.docset_max_len, args.device)
