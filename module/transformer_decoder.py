...
from module.neural import PositionwiseFeedForward,PositionwiseFeedForward2
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, heads, d_ff, dropout, use_z2):
    ...
    def forward(self, inputs, memory_bank, z, z2, src_pad_mask, tgt_pad_mask, layer_cache=None, step=None):
        """
        Args:
        ...
        if self.use_z2:
            output = self.feed_forward(self.drop(word_context) + z + query - z2)
        else:
            output = self.feed_forward(self.drop(word_context) + z + query)
...
class TransformerDecoder(nn.Module):
    def __init__(self, num_layers, cand_num, use_z2, d_model, heads, d_ff, dropout, embeddings, generator):
        ...
        self.transformer_layers = nn.ModuleList(
            [TransformerDecoderLayer(d_model, heads, d_ff, dropout, use_z2)
        ...
        self.feed_forward2 = PositionwiseFeedForward2(d_model*cand_num, d_ff,d_model, dropout)
        self.feed_forward3 = PositionwiseFeedForward2(d_model*cand_num, d_ff,d_model, dropout)

    def forward(self, tgt, memory_bank, z, z2, state, memory_lengths=None,
        ...
        z = z.view(src_batch, -1)

        z = self.feed_forward2(z)

        z = z.unsqueeze(1).expand(z.size(0), tgt_len, z.size(1))

        z2 = z2.view(src_batch, -1)

        z2 = self.feed_forward3(z2)

        z2 = z2.unsqueeze(1).expand(z2.size(0), tgt_len, z2.size(1))
